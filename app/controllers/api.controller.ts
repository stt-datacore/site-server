...
router.get('/offer_info2', async (req: Request, res: Response, next) => {
	try {
		let rawData = await DataCoreAPI.sttGetBeamableStore(getLogDataFromReq(req));
...
router.get('/sales', async (req: Request, res: Response, next) => {
	try {
		let apiResult = await DataCoreAPI.getSales();
		res.status(apiResult.Status).send(apiResult.Body);
	} catch (e) {
		next(e);
	}
